...
// META: title=Link header with comma-separated targets correctly identifies payment-method-manifest link
...
promise_test(async t => {
  ...
  const pmiUrl = createPaymentMethodIdentifierUrl(testId, {
    link: `<${otherUrl}>; rel="other", <${manifestUrl}>; rel="payment-method-manifest"`
  });
  ...
  assert_true(logs.length >= 2, 'Browser must issue 2 server requests (HEAD for PMI, GET for Manifest)');
  ...
  assert_equals(logs[1].endpoint, 'payment-method-manifest', 'Second request must hit correct manifest URL');
