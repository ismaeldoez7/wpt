...
promise_test(async t => {
  const testId = token();
  const manifestUrl1 = createPaymentMethodManifestUrl(testId, { status: 200 });
  const manifestUrl2 = createPaymentMethodManifestUrl(testId, { status: 200 });
  ...
  assert_equals(logs[0].endpoint, 'pmi', 'Request must hit PMI URL');
