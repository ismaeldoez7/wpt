...
// META: title=Same-site cross-origin redirects on initial HEAD request are permitted
// META: script=/common/utils.js
// META: script=/payment-method-manifest/resources/helpers.js

promise_test(async t => {
  const testId = token();
  const targetUrl = createPaymentMethodIdentifierUrl(testId, { host: '{{hosts[][www]}}:{{ports[https][0]}}' });
  ...
  assert_true(logs.length >= 3, 'Browser must follow same-site redirect and fetch manifest');
  ...
  assert_equals(logs[1].endpoint, 'pmi', 'Second request must hit redirected PMI URL on same-site subdomain');
