...
promise_test(async t => {
  ...
  const targetUrl = createPaymentMethodIdentifierUrl(testId, {
    link: `<payment-method-manifest.py?id=${testId}>; rel="payment-method-manifest"`,
  });
  const pmiUrl = createPaymentMethodIdentifierUrl(testId, { redirect_location: targetUrl });
  ...
  // 3. GET to manifest URL resolved relative to targetUrl
  const logs = await waitForServerAccessLogs(t, testId, 3);

  assert_true(logs.length >= 3, 'Browser must follow redirect and fetch manifest via relative Link header');
  assert_equals(logs[0].endpoint, 'pmi', 'First request must hit initial PMI URL');
  assert_equals(logs[0].method, 'HEAD', 'First request must use HEAD method');

  assert_equals(logs[1].endpoint, 'pmi', 'Second request must hit post-redirect target PMI URL');
  assert_equals(logs[1].method, 'HEAD', 'Second request must use HEAD method');

  assert_equals(logs[2].endpoint, 'payment-method-manifest', 'Third request must fetch manifest');
  ...
  assert_equals(logs[2].url, expectedManifestUrl, 'Manifest URL must be resolved relative to final post-redirect PMI URL');
